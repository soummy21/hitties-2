...
public class Blob : MonoBehaviour {
    private class PropagateCollisions : MonoBehaviour {
        void OnCollisionStay2D(Collision2D collision) {
            transform.parent.SendMessage("setCheck",true);
        }
        void OnCollisionExit2D(Collision2D collision)
        {
            transform.parent.SendMessage("setCheck",false);
        }
    }
    ...
    void CreateReferencePoints() {
        ...
        {
            AttachWithSpringJoint(connectionPoints[i], gameObject);
        }
    ...
    public void SetCheck(bool checkValue)
    {
        check = checkValue;
    }
